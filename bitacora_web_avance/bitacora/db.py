...
def _find_date_column(columns: list[str]) -> str | None:
    ...
    for column in columns:
        if "fecha" in column.lower() or column.lower().startswith("f_"):
            return column
    return None
...
def obtener_registros_combustible(
    ...
    if not _IDENTIFIER_RE.fullmatch(source):
        raise DatabaseConfigurationError(
            "COMBUSTIBLE_TABLE debe tener formato esquema.objeto o base.esquema.objeto."
        )

    date_column = _find_date_column(_query_columns(f"SELECT TOP 0 * FROM {source}"))
...
def obtener_reporte_combustible(
    ...
    if not _IDENTIFIER_RE.fullmatch(procedure):
        raise DatabaseConfigurationError(
            "El procedimiento configurado para el reporte no tiene un formato válido."
        )
