...
def _connection_string(database_name: str | None = None) -> str:
    ...
    database = (
        database_name
        or os.getenv("DB_NAME", "dim_sis_puerto_v1")
    ).strip()
    ...
    if not server:
        raise DatabaseConfigurationError(
            "Falta DB_SERVER en el archivo .env."
        )

    if not database:
        raise DatabaseConfigurationError(
            "Falta DB_NAME en el archivo .env."
        )
...
def execute_procedure(
    ...
    *,
    ...
    sql = (
        f"EXEC {procedure} {assignments}"
        if assignments
        else f"EXEC {procedure}"
    )
...
# Se utiliza, entre otros casos, para consultar los buques
# industriales y artesanales activos.
# ==========================================================
...
def execute_query(
    ...
    y devuelve las filas como lista de diccionarios.
    """
...
# ==========================================================
# FIN CONSULTAS SQL DIRECTAS
# ==========================================================


def validated_procedure(
    env_name: str,
    required: bool = True,
) -> str:
    """
    Valida y obtiene nombre de procedimiento
...
def validated_parameter(
    ...
    """
    Valida y obtiene nombre de parámetro
    desde variable de entorno.
    """
    ...
    if not _PARAMETER_RE.fullmatch(value):
        raise DatabaseConfigurationError(
            f"{env_name} debe ser un nombre "
            "de parámetro como @usuario."
        )
...
def first_value(
    ...
    de un diccionario para las claves dadas.
    """
...
def is_missing_object_error(
    ...
    """
    Detecta si la excepción es por
    procedimiento/vista no encontrado.
    """
    ...
    return (
        "2812" in message
        or
        "no se encontró el procedimiento almacenado"
        in message
    )